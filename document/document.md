## 一言サービスコンセプト
![alt text](<スクリーンショット 2024-12-29 13.47.42-1-1.png>)
...
- 収入額と支出額が表示される
- 支出額はカテゴロー毎に金額が表示される
- 支出額はカテゴロー毎にグラフが表示される
- 支出額はカテゴロー毎に一覧画面をもっている
...
## テーブル定義書
### ER図
![alt text](<スクリーンショット 2024-12-29 13.42.52-1.png>)
...
| sector_id | 業種ID  | VARCHAR(50)  |     | TRUE |        | FOREIGN |
| name      | 名前    | VARCHAR(100) |     | TRUE | TRUE   |         |
| equity    | 純資産   | VARCHAR(50)  |     | TRUE |
...
|--------------|--------|-----------|-----|------|--------|---------|
| id           | ID     | INT       |     | TRUE |        | PRIMARY |
| user_id      | ユーザーID | BIGINT    |     | TRUE |        | FOREIGN |
| company_code | 銘柄コード  | BIGINT    |     | TRUE |        | FOREIGN |
| date         | 購入日    | DATE      |     | TRUE |        |         |
| quantity     | 口数     | INT       |     | TRUE |        |         |
| created_at   | 作成日時   | TIMESTAMP |     | TRUE |        |         |
| updated_at   | 更新日時   | TIMESTAMP |     | TRUE |

## システム構成図
※アプリケーションレイヤーのみ
![alt text](<スクリーンショット 2024-12-29 23.43.52-1.png>)
