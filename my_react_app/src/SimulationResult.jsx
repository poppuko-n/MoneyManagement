...
const SimulationResult = () => {
  ...
  const calculateTotalProfitLoss = (period) => {
    return results.reduce((total, item) => {
      const initialInvestment = calculateInitialInvestment(item, period);
      const evaluationValue = item[selectedType][`${period}_ago`];
      return total + (evaluationValue - initialInvestment);
    }, 0);
  };
